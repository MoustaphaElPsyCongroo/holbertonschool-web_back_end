...
if __name__ == "__main__":
    ...
    r = requests.post('http://0.0.0.0:3456/api/v1/auth_session/login',
    ...
    try:
        ...
        r_user_me = requests.get(
            'http://0.0.0.0:3456/api/v1/users/me', cookies={'_my_session_id': cookie_session_id})
        ...
        if r_user_me.headers.get('content-type') != "application/json":
            print("Wrong content type: {}".format(
                r_user_me.headers.get('content-type')))
            exit(1)
